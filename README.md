...
`forecast_comparison.png`는 테스트 구간의 처음 200개 시점을 대상으로 전국 전력수요, 서울 기온, 세 모델의 예측값을 비교한 이미지입니다. 위쪽 그래프의 실선은 전력수요, 주황색 점선은 서울 기온이며, 오른쪽 보조축으로 기온을 읽습니다. 아래쪽 막대그래프에서는 MAE와 RMSE를 모델별로 비교합니다. 세 모델 모두 매 시간 직전까지의 실제 전력수요를 사용해 다음 한 시간을 예측하는 동일한 walk-forward 방식으로 평가합니다.
...
현재 95% 예측 구간은 각 모델의 검증 구간 잔차 표준편차를 이용한 근사값이며, 평가 정답을 사용하지 않아 데이터 누출을 피했습니다. ASOS의 강수량·일조·일사 결측치는 0으로 처리하고, 기온·풍속·습도만 연속값으로 보간했습니다. `forecast_metrics.csv`에는 MAE, RMSE, MAPE와 함께 실제값 표준편차, 예측값 표준편차, 표준편차 비율, 95% 구간 포함률이 저장됩니다. 향후에는 quantile loss 또는 Monte Carlo dropout을 이용한 불확실성 추정을 적용할 수 있습니다.
