...
def main():
    ...
    story = [
        ...
        paragraph('National Power Demand and Regional Weather in 2025', 'KoreanHeading'),
        paragraph('본 보고서는 한국전력거래소의 2025년 시간별 전국 전력수요량과 서울·인천·대전·대구·광주·부산·제주 ASOS 시간별 기상관측자료를 결합하여 전력수요를 예측한 결과를 정리한 문서입니다. ARIMA, LSTM, Transformer 세 가지 모델을 동일한 시간순 데이터와 동일한 one-step-ahead 평가 방식으로 비교했습니다.'),
        ...
        paragraph('사용한 원자료는 한국전력거래소 전국 전력수요량 파일과 7개 지역 ASOS 관측 파일입니다. 전력 파일의 1시부터 24시까지 열을 시간별 행으로 변환하고, 각 지역 ASOS의 일시 열과 정확히 일치시키어 지역별 기상 변수를 구성했습니다. 이 지역별 변수를 전국 수요를 설명하는 다변량 입력으로 사용합니다.'),
